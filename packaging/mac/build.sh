...
# .app bundle for Seven Kingdoms: Ambition.  The prefix is embedded inside the
# bundle so the package is fully relocatable.
...
# TODO: Boost Serialisation package name on Mac/brew
...
#
# Note: The music is not included.  Download the music archive from
# https://www.7kfans.com/downloads/7kaa-music-2.15.tar.bz2 and copy the MUSIC/
# directory into data/.

STRIP=strip

REPO="$(cd "$(dirname "$0")/../.." && pwd)"
BIN="$REPO/build/src/7k-ambition"
DATA="$REPO/data"
...
VERSION=$(git describe --always --dirty --tags 2>/dev/null || cat VERSION)
[ -n "$VERSION" ] || {
    echo "Could not extract version from either git or VERSION" >&2; exit 1;
}
BUNDLE_ID="org.7k-ambition.7k-ambition"
...
RDIR="$CDIR/Resources"
EXEC="$XDIR/7k-ambition"
ICNS="$RDIR/7k-ambition.icns"
...
if [ "$WITH_MULTIPLAYER" = yes ]; then
  ...
  #
  # configure.ac requires fpu_type=387 (x86 only) to enable multiplayer.  arm64
  # lacks that mode, so this build would desync against x86 clients, both online
  # and locally.  libcurl is left out to mitigate that by removing the online
  # lobby; online connect attempts show: "Unable to connect to the 7kfans
  # service. (No libcurl)"
  # -------------------------------------------------------------------------
...
echo "Building icon..."
TMPICONSET=$(mktemp -d)
ICONSET="$TMPICONSET/7k-ambition.iconset"
mkdir -p "$ICONSET"
