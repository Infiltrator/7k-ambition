...
void Town::disp_train_menu(int refreshFlag)
{
	...
	else if( refreshFlag == INFO_REPAINT )
	{
		...
		for(int i=1; i<=MAX_TRAINABLE_SKILL; i++)
		{
			...
			INFO_X2, y+(Ambition::Config::enhancementsAvailable() ? TRAIN_BUTTON_HEIGHT : BUTTON_ACTION_HEIGHT)-1,
...
int Town::detect_train_menu()
{
	...
	if( button_cancel3.detect(Ambition::Input::getKeyEvent(Ambition::Input::Action::Common_Cancel)) || (!waitFlag && mouse.any_click(1)) )		// press the cancel button or right click
	{
		// ##### begin Gilbert 26/9 ########//
		se_ctrl.immediate_sound("TURN_OFF");
		// ##### end Gilbert 26/9 ########//
...
int Town::detect_auto_menu(int modeCollectTax)
{
	...
	if( button_cancel2.detect(Ambition::Input::getKeyEvent(Ambition::Input::Action::Common_Cancel)) || rc )
	{
		// ##### begin Gilbert 26/9 ########//
		se_ctrl.immediate_sound("TURN_OFF");
		// ##### begin Gilbert 26/9 ########//
...
int Town::detect_spy_menu()
{
	...
	if( button_cancel.detect(Ambition::Input::getKeyEvent(Ambition::Input::Action::Common_Cancel)) || mouse.any_click(1) )		// right click to cancel
	{
		info.disp();
		return 1;
	}
