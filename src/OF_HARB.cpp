...
void FirmHarbor::disp_build_menu(int refreshFlag)
{
	...
	else if( refreshFlag == INFO_REPAINT )
	{
		...
		Ambition::Draw::doneButtonText(button_cancel);
...
int FirmHarbor::detect_build_menu()
{
	...
	if( button_cancel.detect(Ambition::Input::getKeyEvent(Ambition::Input::Action::Common_Cancel)) || (!waitFlag && mouse.any_click(1)) )		// press the cancel button or right click
	{
		harbor_menu_mode = HARBOR_MENU_MAIN;
		info.disp();
		// ###### begin Gilbert 26/9 #######//
	...
	{
		if (Ambition::Building::enqueueProduction(this, -1, button_unit_id[queue_ship_selected], 1)) {
			se_ctrl.immediate_sound("TURN_ON");
			info.update();
			return 1;
		}
	...
	{
		if (Ambition::Building::enqueueProduction(this, queueOrder, button_unit_id[queue_ship_selected], HARBOR_BUILD_BATCH_COUNT)) {
			se_ctrl.immediate_sound("TURN_ON");
			info.update();
			return 1;
		}
