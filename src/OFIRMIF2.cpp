...
void Firm::disp_spy_menu(int refreshFlag)
{
	...
	if( refreshFlag == INFO_REPAINT )
	{
		...
		if( firm_menu_mode == FIRM_MENU_SPY )
		{
			...
			if( can_player_spy_capture() )
			{
				...
				Ambition::Draw::buttonKeybind(Ambition::Input::getKeyEvent(Ambition::Input::Action::Spy_Capture), button_capture);
			...
			if( nation_recno && nation_recno != nation_array.player_recno )
			{
				...
				Ambition::Draw::buttonKeybind(Ambition::Input::getKeyEvent(Ambition::Input::Action::Spy_StealReports), button_view_secret);
				 ...
				 firm_res[firm_id]->need_overseer )
			{
				...
				Ambition::Draw::buttonKeybind(Ambition::Input::getKeyEvent(Ambition::Input::Action::Spy_Assassinate), button_assassinate);
		...
		else if( firm_menu_mode == FIRM_MENU_SELECT_BRIBER )
		{
			...
			Ambition::Draw::buttonKeybind(Ambition::Input::getKeyEvent(Ambition::Input::Action::Spy_Bribe), button_bribe);
		}
		else
			err_here();
		...
		Ambition::Draw::buttonKeybind(Ambition::Input::getKeyEvent(Ambition::Input::Action::Common_Cancel), button_cancel);
	}
	...
	//#### end alex 20/3 ####//
	{
		...
		if( spyPtr->spy_skill >= MIN_VIEW_SECRET_SPYING_SKILL )
		{
			...
			Ambition::Draw::buttonKeybind(Ambition::Input::getKeyEvent(Ambition::Input::Action::Spy_StealReports), button_view_secret, refreshFlag);
		}
		else
...
void Firm::detect_spy_menu()
{
	...
	if( firm_menu_mode == FIRM_MENU_SPY )
	{
		...
		else if( button_spy_action.detect(Ambition::Input::getKeyEvent(Ambition::Input::Action::Spy_ChangeMission)))		// set action mode
		{
			if( !remote.is_enable() )
			{
		...
		else if( button_capture.detect(Ambition::Input::getKeyEvent(Ambition::Input::Action::Spy_Capture)) )
		{
		...
		else if( button_view_secret.detect(Ambition::Input::getKeyEvent(Ambition::Input::Action::Spy_StealReports)) )
		{
	...
	{
		if( button_bribe.detect(Ambition::Input::getKeyEvent(Ambition::Input::Action::Spy_Bribe)) )
		{
			action_spy_recno = spyPtr->spy_recno;
...
void Firm::disp_spy_button(int x, int y, int refreshFlag)
{
	...
	if( player_spy_count > 0 )
	{
		...
		if( refreshFlag == INFO_REPAINT )
		{
			...
			Ambition::Draw::buttonKeybind(Ambition::Input::getKeyEvent(Ambition::Input::Action::Spy_Menu), button_spy_menu);
		}
...
int Firm::detect_spy_button()
{
	if( player_spy_count>0 )
	{
		if( button_spy_menu.detect(Ambition::Input::getKeyEvent(Ambition::Input::Action::Spy_Menu)) )
		{
	...
	if( button_capture.detect(Ambition::Input::getKeyEvent(Ambition::Input::Action::Spy_Capture)) && can_worker_capture(nation_array.player_recno) )
	{
		// ##### begin Gilbert 24/6 ##########//
		if( !remote.is_enable() )
		{
