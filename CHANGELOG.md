...
- Added access to cycling through all speeds 1–10,922.3* using the `-` and `=`
  keys.  These can be rebound using the `KEYEVENT_SPEED_DECREASE_STANDARD` and
  `KEYEVENT_SPEED_INCREASE_STANDARD` bindings in the config.txt file.
- * With the exception of speed 33, which is used internally to represent
  unlimited speed, and will be skipped over.
- Added access to cycling through all the intermediate speeds (there are three
  steps from each standard speed level to the next; and the two intermediate
  speeds are shown as .3 and .7).  These can be accessed using the `shift+-` and
...
- Fixed the display of non-standard characters on Windows (especially Júlia
  Rosell Saldaña's name on the What's New screen).
...
- Added button help text to the Egyptian, Mughal, and Zulu Greater Being spell
  buttons.
- Fixed the Encyclopaedia slideshow showing a blank image until the mouse was
  moved.
- Fixed the grammar in the "Desperate Bowmen" scenario description.
